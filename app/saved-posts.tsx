...
export default function SavedPostsFeed() {
  ...
  const fetchData = useCallback(async (action: string, p: number) => {
    ...
    try {
      const result = await rpc_functions_fetch("user_saved_posts_function", { page: p }, p);
  ...
  return (
    <View style={[styles.root, { paddingTop: topPad, paddingBottom: bottomPad }]}>
      ...
      {posts_data.length === 0 && noResults ? (
        <NoResultView text_render="There are no saved posts yet" />
      ) : posts_data.length === 0 ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <PostView posts_data={[item]} action="unsave" onDeleteSuccess={(id) => handleUnsave(id)} />
          )}
